...
public class BlindCaveTetra extends AbstractIchthyologySchoolingFish {

	public BlindCaveTetra(EntityType<? extends AbstractIchthyologyFish> p_30341_, Level p_30342_) {
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(5, new BlindCaveTetraFollowFlockLeaderGoal(this));
		//this.goalSelector.addGoal(5, new BoidGoal(this, 5f, 5f, 1 / 20f, 1 / 20f));
		//this.goalSelector.addGoal(3, new StayInWaterGoal(this));
		//this.goalSelector.addGoal(2, new LimitSpeedAndLookInVelocityDirectionGoal(this, 0.05f, 0.1f));
	}

	class BlindCaveTetraFollowFlockLeaderGoal extends Goal {
		@SuppressWarnings("unused")
		private static final int INTERVAL_TICKS = 200;
		private final BlindCaveTetra mob;
		private int timeToRecalcPath;
		private int nextStartTick;

		public BlindCaveTetraFollowFlockLeaderGoal(BlindCaveTetra p_25249_) {
			this.mob = p_25249_;
			this.nextStartTick = this.nextStartTick(p_25249_);
		}

		protected int nextStartTick(BlindCaveTetra p_25252_) {
		...
		public boolean canUse() {
			if (this.mob.hasFollowers()) {
				...
				Predicate<BlindCaveTetra> predicate = (p_25258_) -> {
					return p_25258_.canBeFollowed() || !p_25258_.isFollower();
				};
				List<? extends BlindCaveTetra> list = this.mob.level().getEntitiesOfClass(this.mob.getClass(), this.mob.getBoundingBox().inflate(8.0D, 8.0D, 8.0D), predicate);
				BlindCaveTetra abstractschoolingfish = DataFixUtils.orElse(list.stream().filter(BlindCaveTetra::canBeFollowed).findAny(), this.mob);
	...
	class BoidGoal extends Goal {
		...
		public Vec3 random() {
			...
			if (Mth.abs((float) velocity.x) < 0.001 && Mth.abs((float) velocity.z) < 0.001)
	...
	public static boolean checkBlindCaveTetraSpawnRules(EntityType<? extends LivingEntity> p_217018_, ServerLevelAccessor p_217019_, MobSpawnType p_217020_, BlockPos p_217021_, RandomSource p_217022_) {
