...
public class PeacockBassBaby extends WaterAnimal implements Bucketable {
	...
	public PeacockBassBaby(EntityType<? extends PeacockBassBaby> entityType, Level level) {
	...
	private boolean isFood(ItemStack pStack) {
		return pStack.getItem() == ModItems.GROUND_FISH;
	}
	...
	private void ageUp() {
		...
		if (level instanceof ServerLevel serverLevel) {
			PeacockBass peacockbass = ModEntityTypes.PEACOCK_BASS.create(this.level());
			if (peacockbass != null) {
				peacockbass.moveTo(this.getX(), this.getY(), this.getZ(), this.getYRot(), this.getXRot());
				peacockbass.finalizeSpawn(serverLevel, this.level().getCurrentDifficultyAt(peacockbass.blockPosition()), MobSpawnType.CONVERSION, (SpawnGroupData)null, (CompoundTag)null);
				peacockbass.setNoAi(this.isNoAi());
				if (this.hasCustomName()) {
					peacockbass.setCustomName(this.getCustomName());
					peacockbass.setCustomNameVisible(this.isCustomNameVisible());
				}
				peacockbass.setPersistenceRequired();
				serverLevel.addFreshEntityWithPassengers(peacockbass);
				this.discard();
			}
		}
	}
